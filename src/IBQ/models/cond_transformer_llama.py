...
from pit.util import instantiate_from_config as pit_instantiate_from_config
...
class Net2NetTransformerPiT(L.LightningModule):
    def __init__(self,
                transformer_config,
                cond_stage_config,
                ae_config,
        ...
        self.first_stage_key = first_stage_key
        self.cond_stage_key = cond_stage_key
        self.init_first_stage_from_ckpt(ae_config, ae_ckpt)
        self.init_cond_stage_from_ckpt(cond_stage_config)
        self.transformer = instantiate_from_config(config=transformer_config)
    ...
    def init_first_stage_from_ckpt(self, ae_config, ae_ckpt):
        config = OmegaConf.load(ae_config)
        checkpoint = torch.load(ae_ckpt, map_location="cpu")["state_dict"]
        model = pit_instantiate_from_config(config.model)
    ...
    @torch.no_grad()
    def encode_to_z(self, x):
        quant_z, indices = self.first_stage_model.quant(x)
        indices = indices.view(quant_z.shape[0], -1).to(dtype=torch.long)
        return quant_z, indices
    ...
    @torch.no_grad()
    def decode_to_img(self, index, zshape):
        index = index.reshape(zshape[0], 1, zshape[2], zshape[3])
        # print(index.shape, torch.min(index), torch.max(index))
        x = self.first_stage_model.dequant(index)
        return x
